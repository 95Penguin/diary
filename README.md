...
- 时间轴：按日期分组并分页加载卡片摘要，使用固定内存窗口保持长时间浏览流畅；支持筛选、收藏、草稿和记录详情，详情返回时保留原浏览位置，再点底部时间轴可回到顶部
- 搜索：使用本地 FTS 全文索引检索正文、后续和标签，短关键词仍兼容精确匹配，并支持按时间范围筛选
...
- 足迹地图：高德地图、叶子标记、区域聚合、时间筛选、地点列表与详情；支持地图手势缩放及双击足迹标记后居中放大
- 地点整理：旧记录补点、相近地点检查、地点别名与坐标隐私处理
- 备份与恢复：采用文件流转的低内存完整 ZIP 备份，支持固定目录自动备份、恢复预览、媒体完整性验证和备份提醒；恢复前自动生成并校验本机安全快照
...
- 本地存储：SQLite、WAL、外键、索引、版本迁移和 30 天回收站；Android 原生数据库句柄意外失效时会自动重建连接
...
冲突优先保留本机版本，避免覆盖本机尚未导出的修改。恢复开始前会在应用文档目录保留
最近的安全快照；备份文件、媒体引用和导入后的数据库关系检查通过后才会完成恢复。

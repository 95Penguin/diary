...
import type { DeletedEntry, Draft, DraftImage, Entry, EntryImage, EntryInput, EntryVersion, FollowUp, ImportResult, JournalBackup, JournalStats, SearchResult } from '@/domain/journal';
...
type EntryAssetInput = { uri: string; width: number; height: number };
...
export async function createEntryWithDetails(
  ...
  await db.withExclusiveTransactionAsync(async (txn) => {
    ...
    for (const [index, image] of images.entries()) await txn.runAsync(
      ...
      createId(), id, image.uri, image.width, image.height, index, now,
    );
...
export async function updateEntryWithDetails(
  ...
  const existing = await db.getAllAsync<{ uri: string }>('SELECT uri FROM entry_images WHERE entry_id = ?', id);
...
export async function importJournalBackup(db: SQLiteDatabase, backup: JournalBackup): Promise<ImportResult> {
  ...
  await db.withTransactionAsync(async () => {
    ...
    if (backup.version >= 6) for (const image of backup.images) {
      ...
      await txn.runAsync(
        `INSERT INTO entry_images (id, entry_id, uri, width, height, sort_order, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(id) DO UPDATE SET uri = excluded.uri, width = excluded.width, height = excluded.height, sort_order = excluded.sort_order`,
        image.id, image.entryId, image.localUri, image.width, image.height, image.sortOrder, image.createdAt,
      );
    }
...
export async function deleteDraft(db: SQLiteDatabase, id: string, keepImages = false) {
  ...
  return keepImages ? [] : draft?.images.map((image) => image.uri) ?? [];
}

      ...
      <Pressable accessibilityRole="none" onPress={(event) => event.stopPropagation()} style={[styles.card, { backgroundColor: readingTheme.background }]}>
        ...
        <View style={styles.buttons}>
          {confirmingDelete ? <>
            ...
            <Pressable onPress={() => void remove()} style={({ pressed }) => [styles.button, { backgroundColor: readingTheme.surface }, pressed && styles.pressed]}><Text style={styles.dangerText}>移入回收站</Text></Pressable>
          </> : <>
            ...
            <Pressable onPress={() => setConfirmingDelete(true)} style={({ pressed }) => [styles.button, { backgroundColor: readingTheme.surface }, pressed && styles.pressed]}><Text style={styles.dangerText}>移入回收站</Text></Pressable>
          </>}
        </View>
...
const styles = StyleSheet.create({
  ...
  primaryText: { color: colors.primary, fontSize: 13, fontWeight: '700', textAlign: 'center' }, dangerText: { flexShrink: 1, color: colors.danger, fontSize: 12, fontWeight: '700', textAlign: 'center' },

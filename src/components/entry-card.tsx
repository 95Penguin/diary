    ...
    <Pressable accessibilityRole="button" accessibilityHint={onLongPress ? '长按可编辑或删除' : undefined} delayLongPress={450} onPress={onPress} onLongPress={onLongPress} style={({ pressed }) => [styles.card, highlighted && styles.highlighted, pressed && styles.pressed]}>
      ...
      <View style={[styles.content, { borderBottomColor: readingTheme.border }]}> 
        ...
        <View style={styles.summary}>
          ...
          {entry.content ? <Text numberOfLines={entry.images.length ? 3 : 5} style={[styles.body, { color: readingBodyStyle.color, fontFamily: readingFontFamily, fontSize: 15 * fontScale, lineHeight: 23 * fontScale * readingBodyStyle.lineHeightMultiplier, letterSpacing: readingBodyStyle.letterSpacing }]}>{entry.content}</Text> : null}
        </View>

...
export function parseJournalBackup(contents: string): JournalBackup {
  ...
  if (backup.format !== 'shishi-journal' || ![1, 2, 3, 4, 5, 6].some((version) => backup.version === version)) throw new Error('unsupported-backup');
  ...
  if (!validEntries || !validFollowUps || !validTags || !validImages || !validVersions || !validSuppressed) throw new Error('invalid-backup');

...
export function createZipBackupFile(backup: JournalBackup, onProgress?: ZipBackupProgress): Promise<{ uri: string; size: number; missingMedia: number }>;
...
export function storeRecoverySnapshotFile(uri: string, maxSnapshots?: number): Promise<{ uri: string; size: number; retained: number }>;

...
export default function MetadataScreen() {
  ...
  const load = useCallback(async () => setUsage(await listMetadataUsage(db)), [db]);
  ...
  return <SafeAreaView edges={['top']} style={[styles.safe, { backgroundColor: readingTheme.background }]}>
    ...
    <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      <Text style={[styles.intro, { color: readingTheme.secondary }]}>“听歌、学习、摸鱼”等状态直接作为标签使用，也会参与筛选和时光总结。</Text>
      <MetadataSection
        title="标签"
    ...
    <Modal visible={Boolean(editing)} transparent animationType="fade" onRequestClose={() => setEditing(null)}>
      <Pressable onPress={() => setEditing(null)} style={styles.overlay}>
        <Pressable onPress={(event) => event.stopPropagation()} style={[styles.editorCard, { backgroundColor: readingTheme.background }]}>
          <Text style={[styles.editorTitle, { color: readingTheme.text }]}>{editing?.item ? '重命名' : '新增'}{editing?.kind === 'tag' ? '标签' : '地点'}</Text>
          ...
          <Text style={[styles.mergeHint, { color: readingTheme.secondary }]}>如果名称已经存在，会自动合并。</Text>
...
const styles = StyleSheet.create({
  ...
  intro: { paddingVertical: spacing.lg, fontSize: 10, lineHeight: 17 },
  ...
  sectionTitle: { fontFamily: fonts.serif, fontSize: 15, fontWeight: '600' }, sectionCount: { marginTop: 2, fontSize: 9 },
  addButton: { paddingHorizontal: spacing.md, paddingVertical: 7, borderRadius: radii.pill }, addText: { color: colors.primary, fontSize: 10, fontWeight: '700' },
  ...
  itemMain: { flex: 1, minWidth: 0, paddingVertical: spacing.sm }, itemName: { fontSize: 12, fontWeight: '600' }, itemCount: { marginTop: 2, fontSize: 9 },
  pin: { paddingLeft: spacing.md, color: colors.textFaint, fontSize: 18 }, pinActive: { color: colors.primary }, remove: { paddingLeft: spacing.md, fontSize: 20, lineHeight: 24 }, empty: { padding: spacing.lg, fontSize: 10, textAlign: 'center' },
  ...
  mergeHint: { marginTop: spacing.sm, fontSize: 9, textAlign: 'center' },

...
import { getEntry, getRandomMemoryEntry, listMemoryEntryIndex, listMemoryTagIndex, listSuppressedMemoryEntryIds, suppressMemoryEntry } from '@/database/journal-repository';
...
import { CompactPillButton } from '@/components/ui/buttons';
...
export default function MemoriesScreen() {
  ...
  const metadataRevision = useJournalDataRevision(['metadata']);
  ...
  const loadedTagRevision = useRef(-1);
  ...
  useFocusEffect(useCallback(() => {
    ...
    const task = InteractionManager.runAfterInteractions(() => {
      void (async () => {
        try {
          if (firstLoad) {
            const initialEntry = await getRandomMemoryEntry(db, new Date().toISOString());
            if (!active) return;
            if (initialEntry) {
              pickedIdRef.current = initialEntry.id;
              setPickedId(initialEntry.id);
              setPickedResult({ id: initialEntry.id, entry: initialEntry, failed: false });
              setLoading(false);
              await new Promise<void>((resolve) => { indexTimer = setTimeout(resolve, 240); });
              if (!active) return;
            }
          }
  ...
  useEffect(() => {
    if (mode !== 'tag' || (tagIndex && loadedTagRevision.current === metadataRevision)) return;
    let active = true;
    const task = InteractionManager.runAfterInteractions(() => {
      if (!active) return;
      setTagsLoading(true);
      void listMemoryTagIndex(db).then((items) => {
        if (!active) return;
        loadedTagRevision.current = metadataRevision;
        setTagIndex(items);
      }).catch(async () => {
  ...
  }, [db, metadataRevision, mode, tagIndex]);
  ...
  useEffect(() => {
    let active = true;
    if (!pickedId || pickedResult?.id === pickedId) return;
    void getEntry(db, pickedId).then((entry) => { if (active) setPickedResult({ id: pickedId, entry, failed: !entry }); }).catch(() => { if (active) setPickedResult({ id: pickedId, entry: null, failed: true }); });
    return () => { active = false; };
  }, [db, pickedId, pickedResult?.id, pickedRetry]);
  const picked = pickedResult?.id === pickedId ? pickedResult.entry : null;
  const pickedLoading = Boolean(pickedId && pickedResult?.id !== pickedId);
  const pickedFailed = Boolean(pickedId && pickedResult?.id === pickedId && pickedResult.failed);
  ...
  return <SafeAreaView edges={['top']} style={[styles.safe, { backgroundColor: readingTheme.background }]}>
    ...
    <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      <View style={styles.modeRow}><CompactPillButton ref={modeButtonRef} accessibilityLabel="选择拾取方式" disabled={!indexReady} onPress={openModePicker} style={[styles.modeButton, { backgroundColor: readingTheme.surface }]}><Text style={styles.modeButtonText}>{modeLabel}</Text><View style={[styles.modeChevron, modePickerVisible && styles.modeChevronOpen]} /></CompactPillButton><Text style={[styles.candidateCount, { color: readingTheme.secondary }]}>{indexReady ? candidates.length ? `${candidates.length} 条可拾起` : '暂无记录' : '正在整理回忆…'}</Text><CompactPillButton accessibilityLabel="再拾一条" disabled={!indexReady} onPress={pickNext} style={styles.shuffleButton}><SymbolView name={{ ios: 'arrow.clockwise', android: 'refresh', web: 'refresh' }} size={17} tintColor="#FFFFFF" /></CompactPillButton></View>
...
function ReviewStrip({ weekEntries, monthEntries, month }: { weekEntries: MemoryEntryIndex[]; monthEntries: MemoryEntryIndex[]; month: number }) {
...
function ReviewMetric({ title, count, media }: { title: string; count: number; media: number }) {
  const { readingTheme } = useAppPreferences();
  return <View style={styles.reviewMetric}><Text style={styles.reviewTitle}>{title}</Text><Text style={[styles.reviewValue, { color: readingTheme.text }]}>{count} 条</Text><Text style={[styles.reviewLabel, { color: readingTheme.secondary }]}>{media} 个媒体</Text></View>;
}

function Heatmap({ entries, year }: { entries: MemoryEntryIndex[]; year: number }) {
  const { readingTheme } = useAppPreferences();
...
const styles = StyleSheet.create({
  ...
  modeRow: { height: 32, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, marginBottom: spacing.sm }, modeButton: { gap: spacing.xs, paddingHorizontal: spacing.sm, backgroundColor: colors.primarySoft }, modeButtonText: { color: colors.primary, fontSize: 10, lineHeight: 14, fontWeight: '700' }, modeChevron: { width: 6, height: 6, marginTop: -2, borderRightWidth: 1.5, borderBottomWidth: 1.5, borderColor: colors.primary, transform: [{ rotate: '45deg' }] }, candidateCount: { flex: 1, color: colors.textFaint, fontSize: 9, textAlign: 'right' }, shuffleButton: { width: 28, paddingHorizontal: 0, backgroundColor: colors.primary }, shuffleText: { color: '#FFFFFF', fontSize: 16, lineHeight: 19, textAlign: 'center', includeFontPadding: false }, tagLoader: { height: 30 }, tagRow: { gap: spacing.xs, paddingBottom: spacing.sm }, tagChip: { paddingHorizontal: spacing.sm, paddingVertical: 4, borderRadius: radii.pill, backgroundColor: colors.surfaceMuted }, tagText: { color: colors.textSecondary, fontSize: 10 }, tagTextActive: { fontWeight: '700' },
